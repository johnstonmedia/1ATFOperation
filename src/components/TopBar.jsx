          ...
          <Link to="/" className="row center" style={{ gap: 14, color: 'inherit' }}>
            ...
            <div className="col" style={{ gap: 2 }}>
              <div className="head" style={{ fontSize: 18, color: '#fff' }}>
                {n.shortName} <span className="dim" style={{ fontSize: 12 }}>// {n.unitName}</span>
              </div>
              <div className="mono accent" style={{ fontSize: 10, opacity: 0.85 }}>
                “{n.quote}”
              </div>
            </div>
          </Link>
          ...
          {user ? (
            <>
              <div className="mono dim" style={{ fontSize: 11, textAlign: 'right' }}>
                <div style={{ color: 'var(--accent)' }}>{user.rank} {user.name}</div>
