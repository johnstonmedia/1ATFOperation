  ...
  return (
    <div>
      <OpsHeader title="Digital Activities" sub="TASKING // DOCUMENT → QUIZ">
        <button
          className="primary"
          onClick={() => setEditing({ id: makeId(), title: '', brief: '', audience: 'all', questions: [], distributed: false, scheduledFor: null })}
        >
          + New activity
        </button>
      </OpsHeader>
          ...
          <div key={t.id} className="panel panel-pad row between center wrap" style={{ gap: 12 }}>
            <div>
              ...
              <div className="mono dim" style={{ fontSize: 11 }}>
                {t.questions.length} items · {t.audience === 'all' ? 'ALL UNITS' : `${t.audience}-COY`}
                {' · '}
                {t.distributed
